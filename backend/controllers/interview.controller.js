...
export const analyzeRezume = async (req, res) => {
    try {
        if (!req.file) {
    ...
    try {
        ...
        if (!role || !experience || !mode) {
            return res.status(400).json({
                message: "Role, Experience and Mode are must required."
            })
        }
        ...
        if (user.credits < 50) {
            return res.status(400).json({
                message: "Not enough credits left, minimum 50 required"
            })
        }
        ...
        const interview = await Interview.create({
            ...
            questions: questionsArray.map((q, index) => ({
                question: q,
                difficult: ["easy", "medium", "easy", "hard", "exteamly hard"][index],
                timeLimit: [50, 70, 40, 90, 150][index],
            }))
        })
...
export const getMyInterviews = async (req,res) => {
    try {
        ...
        console.log(error)
        return res.status(500).json({ message: `failed to finish current user interview ${error}` })
    }
}
